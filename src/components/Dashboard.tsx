  ...
  const StatCard: React.FC<{
    ...
    <Card className="animate-fade-in hover:shadow-lg transition-shadow">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className={`p-2 rounded-lg ${color === 'construction' ? 'construction-gradient' : 'steel-gradient'}`}>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          subtitle={`${Math.round((stats.completedTasks / stats.totalTasks) * 100) || 0}% completado`}
        />
        <StatCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="animate-slide-up">
          ...
          <CardContent>
            <div className="space-y-4">
              {tasks.filter(t => t.status === 'pending' || t.status === 'in-progress').slice(0, 4).map((task) => {
                const project = projects.find(p => p.id === task.projectId);
                return (
                  <div key={task.id} className="border-l-4 border-construction pl-4 py-2">
